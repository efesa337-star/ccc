...
async def cmd_ignore(message: Message, user: dict[str, Any]):
    ...
    await answer(message, t(user["lang"], "ignore_added", n=await db.ignore_count(user["id"])))
...
async def cmd_ignorelist(message: Message, user: dict[str, Any]):
    await answer(message, await ignore_list_text(user))
...
async def cmd_pm(message: Message, user: dict[str, Any], command: CommandObject, bot: Bot):
    ...
    try:
        head = await bot.send_message(
            peer["id"], t(peer["lang"], "pm_in_head"),
            link_preview_options=NO_PREVIEW, parse_mode=None,
        )
        if text:
            body, ents = textutil.compose(text, [])
            await bot.send_message(
                peer["id"], body, entities=ents, parse_mode=None,
        ...
        foot = await bot.send_message(
            peer["id"], t(peer["lang"], "pm_in_foot"),
    ...
    for mid in (head.message_id, foot.message_id):
        await db.add_pm(peer["id"], mid, user["id"])
    await answer(message, t(lang, "pm_sent"))
